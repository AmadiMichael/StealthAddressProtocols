...
def generate_stealth_address_from_stealth_meta_address(stealth_meta_address: [PlainPoint2D, PlainPoint2D]) -> (PlainPoint2D, PlainPoint2D, int):
...
def get_stealth_address_private_key(stealth_public_key: PlainPoint2D, ephemeral_public_key: PlainPoint2D, view_tag: int) -> int:
    # calculate shared secret
    shared_secret = multiply(ephemeral_public_key, global_view_priv_key)
    ...
    calc_stealth_public_key = add(multiply(G, bytes_to_int(Q)), global_spend_public_key)
    # compare stealth public keys
    if calc_stealth_public_key != stealth_public_key:
        return 0
    
    # get the priv key
    stealth_priv_key = (bytes_to_int(Q) + global_spend_priv_key) % N
...
global_view_priv_key = randomPrivateKey()
global_spend_priv_key = randomPrivateKey()
global_view_public_key = privtopub(int_to_bytes(global_view_priv_key))
global_spend_public_key = privtopub(int_to_bytes(global_spend_priv_key))
global_stealth_meta_address = [global_view_public_key, global_spend_public_key]
